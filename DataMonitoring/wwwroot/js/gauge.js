(function() {
  ...
  GaugePointer = (function(superClass) {
    ...
    GaugePointer.prototype.render = function() {
      ...
      if (this.gauge.options.limitMax) {
        if (this.displayedValue > this.maxValue)
	  ...
	  startX = Math.round((this.length / 2) * Math.cos(angle));
	  startY = Math.round((this.length / 2) * Math.sin(angle));
      //startX = Math.round(this.strokeWidth * Math.cos(angle - Math.PI / 2));
      //startY = Math.round(this.strokeWidth * Math.sin(angle - Math.PI / 2));
      //endX = Math.round(this.strokeWidth * Math.cos(angle + Math.PI / 2));
	  //endY = Math.round(this.strokeWidth * Math.sin(angle + Math.PI / 2));
	  endX = Math.round((this.length / 2) * Math.cos(angle-this.strokeWidth));
	  endY = Math.round((this.length / 2) * Math.sin(angle-this.strokeWidth));
      //this.ctx.beginPath();
	  ...
	  //this.ctx.beginPath(); 
	  //this.ctx.fillStyle = "#00FFFFFF";
	  //this.ctx.arc(0,0,this.length/2,Math.PI,Math.PI*2,false);
	  //this.ctx.fill();
	  ...
	  this.ctx.font = this.options.font || "10px Times";
      this.ctx.fillStyle = this.options.textColor;
      this.ctx.textBaseline = "bottom";
      this.ctx.textAlign = "center";
	  this.ctx.fillText(formatNumber(this.displayedValue),0,10);
  ...
  Gauge = (function(superClass) {
    ...
    Gauge.prototype.paddingTop = 0.01;
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        if (value.label !== void 0) {
          ...
          if ((!this.options.limitMin || value >= this.minValue) && (!this.options.limitMax || value <= this.maxValue)) {
			...
			this.ctx.fillStyle = staticLabels.colors[j] || "#000000";
            this.ctx.rotate(rotationAngle);
            this.ctx.fillText(formatNumber(value, staticLabels.fractionDigits), 0, -radius - this.lineWidth / 2);
			this.ctx.fillText(staticLabels.infos[j],0,(-radius - this.lineWidth / 2) - 20);
			this.ctx.rotate(-rotationAngle);
			
			var currentDivision = parseFloat(value);
			var range = parseFloat(this.maxValue) - parseFloat(this.minValue);
			var lineWidth = range / 400;
			var divWidth = lineWidth * 1;
	    	var divLength = 0.7;
			var scaleMutate = (this.lineWidth / 2) * (1 - divLength);
			tmpRadius = (this.radius * this.options.radiusScale) + scaleMutate;
              var divColor = staticLabels.colors[j] || "#000000";
