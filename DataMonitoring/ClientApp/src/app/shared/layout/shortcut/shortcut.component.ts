...
export class ShortcutComponent implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  ...
  constructor(private layoutService: LayoutService, private router: Router, private renderer: Renderer2, private el: ElementRef)
  {}
  ...
  listen() {
    this.layoutSub = this.layoutService.subscribe((store) => {
      ...
      if (store.shortcutOpen) {
        this.documentSub = this.renderer.listen('document',
          'mouseup',
          (event) => {
